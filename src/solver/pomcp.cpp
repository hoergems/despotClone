...
double POMCP::Rollout(State* particle, int depth, const DSPOMDP* model,
	...
	auto rolloutStrategy = prior->getRolloutStrategy();
	if (rolloutStrategy)
		return rolloutStrategy(particle, model);
	...
	cout << "rollout reward: " << reward << endl;
