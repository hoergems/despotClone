...
VNode* DESPOT::Trial(VNode* root, RandomStreams& streams,
	...
	do {
		...
		double upper = cur->upper_bound();
		double lower = cur->lower_bound();
...
ValuedAction DESPOT::OptimalAction(VNode* vnode) {
	...
	for (ACT_TYPE action = 0; action < vnode->children().size(); action++) {
		...
		cout << "act: " << action << ", val: " << qnode->lower_bound() << endl;
	...
	/**if (vnode->default_move().value > astar.value) {
		cout << "DEFAULT BETTER" << endl;
		cout << "val default: " << vnode->default_move().value << ", astar.value: " << astar.value << endl;
...
double DESPOT::WEU(VNode* vnode, double xi) {
	...
	/**double gap = Gap(vnode);
	double gapRoot = Gap(root);
	double weight = vnode->Weight();
	cout << "upper_bound: " << vnode->upper_bound() << endl;
	cout << "lower_bound: " << vnode->lower_bound() << endl;
	cout << "gap: " << gap << endl;
	cout << "gapRoot: " << gap << endl;
	cout << "weight: " << weight << endl;
	cout << "xi: " << xi << endl;
	return gap - xi * weight * gapRoot;*/
	return Gap(vnode) - xi * vnode->Weight() * Gap(root);
}
