...
typedef std::function<double(const State *, const DSPOMDP*)> RolloutStrategy;
...
public:
	...
	inline virtual void exploration_constant(double constant) {
		exploration_constant_ = constant;
	}

	inline virtual double exploration_constant() const {
	...
	virtual RolloutStrategy getRolloutStrategy() const {
		return nullptr;
	}
};
